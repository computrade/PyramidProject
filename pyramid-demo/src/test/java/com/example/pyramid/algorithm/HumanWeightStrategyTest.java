...
public abstract class HumanWeightStrategyTest<T extends HumanEdgeWeightStrategy> {
	...
	public void testByLevel() {
		...
		for (int i = 0; i < level + 1; i++) {
			actualWeights[i] = instance.getHumanWeight(level, i, PERSON_WEIGHT);
		}
	...
	@Test
	public void testEdgeByLevel() {
		
		double actualedgeWeight = instance.getHumanEdgeWeight(level, PERSON_WEIGHT);
		
		Assert.assertEquals(expectedWeights[0], actualedgeWeight, Double.MIN_VALUE);
	}
